...
- `ArraySampleParquetRawDatasetBuilder.open_session(...)`: sample별 raw parquet를 쓰는 out-of-order build session을 엽니다.
...
- `builder.compact()`: raw sample 파일들을 최종 part parquet로 묶고 manifest를 생성합니다.
...
중간에 프로세스가 종료되면 direct builder는 commit log에 기록된 part만 인정하고, raw builder는 `raw_samples.jsonl`에 기록된 sample만 완료로 인정합니다. resume 시 `.tmp` 파일은 삭제됩니다.
...
현재 권장 ingest 경로는 raw builder입니다. raw builder에서는 완료 여부를 `raw_samples.jsonl` commit log로 판단하고, `pending_sample_ids()`로 아직 남은 sample을 확인합니다. supervisor가 pending sample을 worker에게 나눠주고, 모든 raw sample이 끝난 뒤 한 프로세스가 `compact()`를 호출해 최종 part를 만듭니다.
...
- `examples/build_array_sample_parquet_example.py`
