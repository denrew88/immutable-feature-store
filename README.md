...
scalar의 최종 shard는 dense-long 하나만 지원합니다. 모든 `(feature_id, sample_id)` 조합을 row로 저장하고 missing은 `mask=0, value=NaN`으로 표현합니다. reader/API에서는 `mask=0`을 missing/null로 해석합니다.
...
필요한 runtime jar는 `java\download_java_libs.ps1`로 받습니다. 현재 Java package 3개 기준으로 Hadoop/Parquet Java writer stack과 Arrow stack은 필요하지 않습니다. `array-binary-shard-java`, `scalar-feature-shard-java`, `array-sample-parquet-java` 모두 DuckDB JDBC와 Jackson만 필요합니다.
...
전체 무결성 체크리스트를 증거 명령과 함께 한 번에 확인하려면 아래 runner를 사용합니다.

```powershell
python python\scripts\run_full_integrity_suite.py --clean-report-dir
```

결과는 `data/full_integrity_suite/full_integrity_report.md`와 `data/full_integrity_suite/full_integrity_report.json`에 기록됩니다. 각 세부 체크 항목은 pass/fail과 함께 어떤 검증 명령을 근거로 통과했는지 남깁니다.
