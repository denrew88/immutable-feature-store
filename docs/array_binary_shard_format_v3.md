...
- `categorical_dictionaries/*.parquet`
...
다만 `time`, `value`를 둘 다 두면 기존 trace 관습과 더 잘 맞고, `sample_flags`의 nonfinite bit도 더 의미 있게 쓸 수 있다.
...
- 원래 label은 별도 dictionary parquet에 저장
...
```json
{
  ...
  "dictionary_path": "categorical_dictionaries/state_code.parquet"
}
```

그리고 `state_code.parquet`는 이런 형태를 갖는다.
...
- `state_code.parquet`와 `event_type.parquet`는 서로 독립적인 예시다
- schema에 `state_code`만 있으면 `state_code.parquet`만 있으면 된다
...
```json
{
  ...
  "point_schema": [
    ...
    },
    {
      ...
      "storage_type": "uint32",
      "logical_type": "categorical",
      "dictionary_path": "categorical_dictionaries/event_type.parquet"
    }
  ],
  "shards": [
    {
...
- `time_dtype`, `value_dtype`는 v2 호환 흔적으로 남을 수 있지만, v3에선 일반 `point_schema`가 authoritative 하다.
...
- 코드 안 변수명은 아직 `sample_row_start` 흔적이 있을 수 있다.
...
| 0 | `0x01` | present |
| 1 | `0x02` | empty |
| 2 | `0x04` | has nonfinite time |
| 3 | `0x08` | has nonfinite value |

주의:

- `0x04`, `0x08`은 `time`, `value` 컬럼이 있을 때만 의미가 있다.
- `time`, `value`가 없는 generic schema에서는 보통 0으로 남는다.
...
- categorical은 `uint32` code + dictionary parquet
