...
javac -encoding UTF-8 -cp "java\lib\duckdb_jdbc-1.1.3.jar" -d java\out @java\sources.txt
```
...
두 jar 모두 thin jar이므로 실행할 때 DuckDB JDBC jar를 classpath에 같이 넣어야 한다.
...
- `java/lib/duckdb_jdbc-1.1.3.jar`는 git 추적 대상이 아니다.
